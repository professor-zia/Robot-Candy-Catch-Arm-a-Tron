...
int analogJ1XPin = 33, analogJ1YPin = 32, analogJ2XPin = 34, analogJ2YPin = 35;

// global variables to store current servo positions, initialize to mid positions (90 deg)
int servo1Pos = 90, servo2Pos = 90, servo3Pos = 90, servo4Pos = 45;

int applyDeadzone(int val, int center = 470, int threshold = 30) {
  if (abs(val - center) < threshold) return center;
  return val;
}

void smoothMove(Servo &servo, int &currentPos, int targetPos, int step = 1) {
  if (currentPos < targetPos) currentPos += step;
  else if (currentPos > targetPos) currentPos -= step;
...
void loop() {
  ...
  analogValue1 = applyDeadzone(analogValue1);
  analogValue2 = applyDeadzone(analogValue2);
  analogValue3 = applyDeadzone(analogValue3);
  analogValue4 = applyDeadzone(analogValue4);
  ...
  //servo1Pos = map(analogValue1, 0, 1024, 0, 180);
  //armServo1.write(servo1Pos);

  int target1 = map(analogValue1, 0, 1024, 0, 180);
  smoothMove(armServo1, servo1Pos, target1);

  servo2Pos = map(analogValue2, 0, 1024, 0, 180);
